<h1 align="center">LVS Task 2 – ONNX Model - Quantized to INT8</h1>
...
<p align="center">
  ...
  dataset. The model for inference in this branch has been optimized to an INT8 format.
</p>
...
This branch has an optimized ONNX INT8 model for edge deployment.

The repository includes:

- Inference using `parking_detector_int8.onnx`
...
├── benchmark/
│   ├── accuracy_int8.py
│   └── benchmark_int8_inference.py
|
├── models/
│   ├── best.pt
│   ├── export_to_onnx.py
│   ├── parking_detector.onnx
│   └── parking_detector_int8.onnx
│
├── data/
|   ├── validation_images/
│   └── validation_labels/
...
```bash
git clone -b feature/inference-int8 https://github.com/scouring/LVS-Task-1.git
cd LVS-Task-1
```
...
```bash
python -m benchmark.benchmark_int8_inference
```
```text
This will calculate:
- latency
